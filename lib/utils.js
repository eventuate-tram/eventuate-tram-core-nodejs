const log4js = require('log4js');

const getLogger = ({ logLevel, title } = {}) => {
  const logger = log4js.getLogger(title || 'Logger');
  if (!logLevel) {
    logLevel = (process.env.NODE_ENV !== 'production') ? 'DEBUG' :' ERROR';
  }

  logger.level = logLevel;
  return logger;
};

const parseMessage = messageStr => {
  try {
    // {"payload":"{\\"message\\":\\"Test kafka subscription\\"}",
    // "headers":{"ID":"0000016d973b1b5a-8116633600000000","PARTITION_ID":"0","DESTINATION":"test-topic","DATE":"Fri, 04 Oct 2019 14:44:56 GMT"}}
    const { payload, headers } = JSON.parse(messageStr);
    const { ID: messageId, DESTINATION: topic, DATE: creationTime, PARTITION_ID: partitionId } = headers;
    const message = {
      messageId,
      topic,
      creationTime,
      partitionId,
      payload
    };

    return { message };
  } catch (error) {
    return { error };
  }
};

const addZeroes = ({ src , width = 16, z = '0', position = 'begin' }) => {

  if (typeof src !== 'string') {
    src = src.toString();
  }

  if (src.length >= width) {
    return src;
  }

  const zeros = new Array(width - src.length + 1).join(z);

  if (position === 'end') {
    return src + zeros;
  }

  return zeros + src;
};

const promisedDelay = (timeout) => {
  console.log(`promisedDelay: ${timeout}`);
  return new Promise(resolve => {
    setTimeout(resolve, timeout);
  });
};

const makeMessageHeaders = ({ messageId, partitionId, topic, creationTime, eventAggregateType, eventType }) => ({
  ID: messageId,
  PARTITION_ID: partitionId.toString(),
  DESTINATION: topic,
  DATE: new Date(creationTime).toUTCString(),
  'event-aggregate-type': eventAggregateType,
  'event-type': eventType
});

module.exports = {
  getLogger,
  parseMessage,
  addZeroes,
  promisedDelay,
  makeMessageHeaders
};