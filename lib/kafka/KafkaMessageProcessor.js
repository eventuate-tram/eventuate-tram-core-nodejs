...
class KafkaMessageProcessor {
  ...
  async process(record) {
    ...
    const { error, event } = parseMessage(record.value);
    ...
    if (!processedEvent) {
      throw new Error('Event handler should return the message')
    }
    const processedRecord = this.notProcessedRecordsByEventId.get(processedEvent.eventId);
