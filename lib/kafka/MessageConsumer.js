...
class MessageConsumer {
  constructor({ fromOffset = 'latest' } = {}) {
    ...
    this.kafkaConsumerGroup = new KafkaConsumerGroup({ connectionString: this.connectionString, fromOffset });
  }
  ...
  async subscribe({ subscriberId, topics, messageHandler }) {
    ...
    const processor = new KafkaMessageProcessor({ handler: messageHandler });
    ...
    this.kafkaConsumerGroup.on('message', async (record) => {
      ...
      try {
        ...
        logger.debug(`Record processed {${subscriberId}}`, record);
