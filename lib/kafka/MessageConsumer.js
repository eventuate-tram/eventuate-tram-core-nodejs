...
const KafkaAggregateSubscriptions = require('./KafkaAggregateSubscriptions');
...
class MessageConsumer {
  constructor({ fromOffset = 'latest' } = {}) {
    ...
    this.kafkaAggregateSubscriptions = new KafkaAggregateSubscriptions({ connectionString: this.connectionString, fromOffset });
  }
  ...
  async subscribe({ subscriberId, topics, messageHandler }) {
    ...
    await this.kafkaAggregateSubscriptions.subscribe({ groupId: subscriberId, topics });

    const processor = new KafkaMessageProcessor({ subscriberId, handler: messageHandler });

    this.kafkaAggregateSubscriptions.on('message', async (record) => {
      ...
      try {
        ...
        this.maybeCommitOffsets(this.kafkaAggregateSubscriptions.consumerGroup, processor);
      } catch (err) {
        logger.error(err);
        reject(err);
      }
  ...
  async unsubscribe() {
    ...
    await this.kafkaAggregateSubscriptions.unsubscribe();
  }
